...
export function createPushMessagingRouter(): Router {
  ...
  router.post('/welcome', asyncHandler(pushMessagingController.sendUserWelcome.bind(pushMessagingController)))

  // POST /push/discount - Invia aggiornamento sconto
  router.post('/discount', asyncHandler(pushMessagingController.sendDiscountUpdate.bind(pushMessagingController)))

  // POST /push/offer - Invia nuova offerta
  router.post('/offer', asyncHandler(pushMessagingController.sendNewOffer.bind(pushMessagingController)))

  // POST /push/chatbot-reactivated - Invia notifica chatbot riattivato
  router.post('/chatbot-reactivated', asyncHandler(pushMessagingController.sendChatbotReactivated.bind(pushMessagingController)))

  // POST /push/test - Test push messaging
  router.post('/test', asyncHandler(pushMessagingController.testPushMessage.bind(pushMessagingController)))
