 ...
 */
export async function createOrderCheckoutLink(params: CreateOrderCheckoutLinkParams): Promise<CreateOrderCheckoutLinkResult> {
  const { customerId, workspaceId, prodotti } = params;
  
  logger.info(`[CREATE_ORDER_CHECKOUT] Creating order checkout link for customer ${customerId}`);
  
  try {
    // Validazione parametri
    if (!customerId || !workspaceId || !prodotti || prodotti.length === 0) {
      throw new Error('Missing required parameters: customerId, workspaceId, or prodotti');
    }
    ...
    if (!customer) {
      throw new Error(`Customer ${customerId} not found in workspace ${workspaceId}`);
    }
...
🔐 Checkout sicuro`;
    ...
    logger.info(`[CREATE_ORDER_CHECKOUT] Checkout link created: ${checkoutToken}, Total: €${totalAmount}`);
    ...
    logger.error(`[CREATE_ORDER_CHECKOUT] Error creating order checkout link:`, error);
    throw error;
  }
}
...
export const createOrderCheckoutLinkFunction = {
  ...
  description: 'Genera link sicuro per checkout con array di prodotti. Da chiamare quando utente vuole confermare un ordine.',
  parameters: {
    type: 'object',
    properties: {
      customerId: { 
      ...
      prodotti: {
        ...
        items: {
          ...
          properties: {
            ...
            descrizione: { type: 'string', description: 'Descrizione prodotto' },
