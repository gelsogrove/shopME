...
import { IProductRepository, PaginatedProducts, ProductFilters } from '../domain/repositories/product.repository.interface';
...
export class ProductRepository implements IProductRepository {
  ...
  async findAll(workspaceId: string, filters?: ProductFilters): Promise<PaginatedProducts> {
    try {
      logger.info('ProductRepository.findAll chiamato con:', { workspaceId, filters });
  ...
  async findByCategory(categoryId: string, workspaceId: string): Promise<Product[]> {
    try {
  ...
  async update(id: string, product: Partial<Product>, workspaceId: string): Promise<Product | null> {
    try {
  ...
  async updateStock(id: string, stock: number, workspaceId: string): Promise<Product | null> {
    try {
      const updatedProduct = await this.prisma.products.update({
        where: {
          id,
  ...
  async updateStatus(id: string, status: ProductStatus, workspaceId: string): Promise<Product | null> {
    try {
      const updatedProduct = await this.prisma.products.update({
        where: {
          id,
