...
export const pushMessagingService = {
  ...
  async sendPushMessage(data: PushMessageData): Promise<boolean> {
    try {
      logger.info(`[PUSH-MESSAGING] 🚀 Sending ${data.type} push to ${data.customerPhone}`)
      
      // 1. Ottieni dati customer completi
      const customer = await this.getCustomerData(data.customerId, data.workspaceId)
      if (!customer) {
        logger.error(`[PUSH-MESSAGING] ❌ Customer ${data.customerId} not found`)
        return false
      }
      ...
      const chatSession = await this.ensureChatSession(customer.id, data.workspaceId)
      
      // 5. 📱 Invio effettivo WhatsApp 
      const whatsappResult = await this.sendWhatsAppMessage(data.customerPhone, message, data.workspaceId)
      ...
      if (whatsappResult.success) {
        ...
        logger.info(`[PUSH-MESSAGING] ✅ Push sent successfully: ${data.type} to ${customer.name}`)
        return true
      } else {
  ...
  async saveOutboundMessage(
    ...
    logger.info(`[PUSH-MESSAGING] 💾 Message saved with status: ${messageStatus}${messageId ? `, messageId: ${messageId}` : ''}`)
  },
  ...
  async trackPushCost(workspaceId: string, customerId: string) {
    ...
    logger.info(`[PUSH-MESSAGING] 💰 Cost tracked: €${PUSH_MESSAGE_COST} for push message`)
  },
  
  /**
   * 📱 Invio effettivo WhatsApp via Business API o Dev Mode
   */
  async sendWhatsAppMessage(phoneNumber: string, message: string, workspaceId: string) {
    try {
      logger.info(`[PUSH-MESSAGING] 📱 Sending WhatsApp message to ${phoneNumber}`)
      ...
      if (!workspace || !workspace.whatsappApiKey || !workspace.whatsappPhoneNumber) {
        logger.error(`[PUSH-MESSAGING] ❌ WhatsApp settings not configured for workspace ${workspaceId}`)
      ...
      if (workspace.whatsappApiKey === 'DEV_MODE_SIMULATOR_TOKEN_FOR_TESTING') {
        logger.info(`[PUSH-MESSAGING] 🔧 DEV MODE: Simulating WhatsApp message send to ${phoneNumber}`)
        ...
        logger.info(`[PUSH-MESSAGING] ✅ DEV MODE: Simulated message sent successfully`, {
      ...
      if (response.ok && responseData.messages?.[0]?.id) {
        logger.info(`[PUSH-MESSAGING] ✅ WhatsApp message sent successfully to ${phoneNumber}`, {
