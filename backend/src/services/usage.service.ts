...
export const usageService = {
  ...
  async trackUsage(data: UsageData): Promise<void> {
    try {
      console.log(`[USAGE-TRACKING] 💰 Adding usage record for workspace ${data.workspaceId}, client ${data.clientId}`);
      ...
      if (!customer) {
        console.log(`[USAGE-TRACKING] ❌ Customer ${data.clientId} not found - no usage tracked`);
        return;
      }

      if (customer.workspaceId !== data.workspaceId) {
        console.log(`[USAGE-TRACKING] ❌ Customer ${data.clientId} belongs to different workspace - no usage tracked`);
        return;
      }

      // Create usage record with 0.5 cents cost
      await prisma.usage.create({
        data: {
          workspaceId: data.workspaceId,
          clientId: data.clientId,
          price: data.price || 0.005, // 0.5 cents as requested by Andrea
        }
      });

      console.log(`[USAGE-TRACKING] ✅ Usage recorded: €${data.price || 0.005} for customer ${customer.name} (${customer.phone})`);
    } catch (error) {
  ...
  async getUsageStats(query: UsageStatsQuery): Promise<UsageStats> {
    try {
      ...
      console.log(`[USAGE-STATS] 📊 Getting usage stats for workspace ${workspaceId} from ${start} to ${end}`);
      ...
      const dailyUsageMap = new Map<string, { cost: number; messages: number }>();
      ...
      const clientUsageMap = new Map<string, {
        clientName: string;
        clientPhone: string;
        cost: number;
        messages: number;
      ...
      const hourlyUsageMap = new Map<number, { messages: number; cost: number }>();
      usageRecords.forEach(record => {
      ...
      console.log(`[USAGE-STATS] ✅ Stats calculated: €${totalCost.toFixed(4)} total, ${totalMessages} messages`);
   ...
   */
  async getUsageSummary(workspaceId: string, days: number = 30): Promise<{
    totalCost: number;
    totalMessages: number;
