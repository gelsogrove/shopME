...
export class EmbeddingService {
  ...
  public static readonly DEFAULT_SEARCH_LIMIT = 200
  ...
  async generateFAQEmbeddings(
    ...
    try {
      ...
      const deletedChunks = await prisma.fAQChunks.deleteMany({
        where: {
          faq: {
            workspaceId: workspaceId,
          },
        ...
        try {
          ...
          for (let i = 0; i < chunks.length; i++) {
            ...
            try {
              ...
              errors.push(
                `FAQ "${faq.question}" chunk ${chunk.chunkIndex}: ${chunkError instanceof Error ? chunkError.message : "Unknown error"}`
              )
            }
          }
        ...
        try {
          ...
          for (let i = 0; i < chunks.length; i++) {
            ...
            try {
              ...
              errors.push(
                `Service "${service.name}" chunk ${chunk.chunkIndex}: ${chunkError instanceof Error ? chunkError.message : "Unknown error"}`
              )
            }
          }
...
${this.generateMultilingualTerms(product.name, product.category?.name)}
          ...
          for (let i = 0; i < chunks.length; i++) {
            ...
            try {
              ...
              errors.push(
                `Product "${product.name}" chunk ${chunk.chunkIndex}: ${chunkError instanceof Error ? chunkError.message : "Unknown error"}`
              )
            }
          }
   ...
   */
  private generateMultilingualTerms(
    productName: string,
    categoryName?: string
  ): string {
    const terms: string[] = []
