...
export class CheckoutController {
  ...
  async submitOrder(req: Request, res: Response): Promise<void> {
    try {
      ...
      // Calculate total
      const totalAmount = prodotti.reduce((sum: number, item: any) => 
        sum + (item.prezzo * item.qty), 0
      );
   ...
   */
  private async sendNotifications(order: any, customer: any, workspace: any): Promise<void> {
    try {
      ...
      const adminEmail = settings?.adminEmail || workspace.notificationEmail;
      ...
      await this.sendWhatsAppNotification(customer.phone, order.orderCode, workspace.id);
   ...
   * Send email to customer
   */
  private async sendCustomerEmail(email: string, order: any, customerName: string): Promise<void> {
    try {
      const emailContent = `
Ciao ${customerName},
...
Il Team
   ...
   * Send email to admin
   */
  private async sendAdminEmail(email: string, order: any, customer: any): Promise<void> {
    try {
      const emailContent = `
...
Accedi al pannello amministrativo per confermare l'ordine.
   ...
   */
  private async sendWhatsAppNotification(phoneNumber: string, orderCode: string, workspaceId: string): Promise<void> {
    try {
      ...
      if (!customer) {
        logger.error(`[CHECKOUT] Customer not found for WhatsApp notification: ${phoneNumber}`);
        return;
      }
      ...
      logger.info(`[CHECKOUT] WhatsApp message saved for ${phoneNumber}: ${orderCode}`);
