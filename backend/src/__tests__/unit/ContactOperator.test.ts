...
describe("ContactOperator", () => {
  ...
  describe("Successful operator contact request", () => {
    beforeEach(() => {
      ;(prisma.customers.findFirst as jest.Mock)
        .mockResolvedValue(mockCustomer)(
          prisma.workspace.findUnique as jest.Mock
        )
        .mockResolvedValue(mockWorkspace)(prisma.customers.update as jest.Mock)
        .mockResolvedValue(mockCustomer)(prisma.message.findMany as jest.Mock)
            ...
            choices: [
              {
                message: {
                  content:
                    "RIASSUNTO AI: Il cliente Mario ha chiesto informazioni sui prodotti. Conversazione cordiale e professionale. Il cliente sembra interessato all'acquisto. Consiglio: Contattare per fornire catalogo dettagliato.",
                },
              },
            ],
          }),
    ...
    it("should handle customer without email gracefully", async () => {
      ...
      expect(logger.warn).toHaveBeenCalledWith(
        expect.stringContaining(
          "has no email - continuing without email requirement"
        )
      )
    ...
    it("should continue execution if email sending fails", async () => {
      const emailService = new EmailService()
      ;(
        emailService.sendOperatorNotificationEmail as jest.Mock
  ...
  describe("Error handling", () => {
    it("should throw error if phone or workspaceId is missing", async () => {
      await expect(
        ContactOperator({ phone: "", workspaceId: "workspace-123" })
      ).rejects.toThrow("Missing phone or workspaceId")

      await expect(
        ContactOperator({ phone: "+393451234567", workspaceId: "" })
      ).rejects.toThrow("Missing phone or workspaceId")
    })
  ...
  describe("AI Summary Generation", () => {
    ...
    it("should include conversation context in AI prompt", async () => {
      ...
      expect(userPrompt).toContain(
        "Cliente: Ciao, vorrei informazioni sui prodotti"
      )
      expect(userPrompt).toContain(
        "Bot: Certo! Abbiamo vari prodotti disponibili"
      )
    })
  })
})
