...
const createOrderHandler = async (
  ...
  try {
    ...
    const { workspaceId, customerId, customerid, items, totalAmount, notes } =
      req.body
    ...
    if (!customer) {
      logger.info(
        `[CREATE-ORDER] Customer ${finalCustomerId} not found, creating temporary customer...`
      )
    ...
    for (const item of orderItems) {
      ...
      // For debugging
      logger.info(
        `[CREATE-ORDER] Processing order item: ${JSON.stringify(item)}`
      )
...
describe("create-order endpoint tests", () => {
  ...
  test("should create order successfully with new format fields (itemType/productCode)", async () => {
    ...
    expect(logger.info).toHaveBeenCalledWith(
      expect.stringContaining("[CREATE-ORDER] Received payload")
    )
  ...
  test("should create order with auto-created customer when customer not found", async () => {
    ...
    expect(logger.info).toHaveBeenCalledWith(
      expect.stringContaining(
        "Customer 0bcf157b-e0ad-45dc-896c-f894cb19ead5 not found, creating temporary customer"
      )
    )
    expect(mockResponse.status).toHaveBeenCalledWith(200)
    expect(mockResponse.json).toHaveBeenCalledWith(
