...
describe('🔐 Secure Token System Integration Tests', () => {
  ...
  beforeAll(async () => {
    ...
    testWorkspace = await prisma.workspace.create({
      data: {
        ...
        slug: 'test-workspace-token-system',
        isActive: true,
        isDelete: false,
  ...
  describe('🔍 Token Validation Tests', () => {
    ...
    it('should reject expired token', async () => {
      // Create token and manually set expiration in the past
      ...
      // Manually expire the token by setting expiration in the past
  ...
  describe('🌐 TOKEN-ONLY System Tests', () => {
    ...
    it('should accept any valid token type for validation (TOKEN-ONLY system)', async () => {
      // Test that token works without specifying type (TOKEN-ONLY system)
      const validation = await secureTokenService.validateToken(universalToken)
      expect(validation.valid).toBe(true)

      // Test that token works when type is explicitly 'any'
      const validationAny = await secureTokenService.validateToken(universalToken, 'any')
      expect(validationAny.valid).toBe(true)

      // Test that token works with different type specifications
      // Note: The TOKEN-ONLY system works when no type is specified or type is 'any'
      // But when specific types are requested, it should fail for 'any' type tokens
      ...
      // For 'any' type tokens, they should fail when specific types are requested
      expect(validationOrders.valid).toBe(false)
      expect(validationProfile.valid).toBe(false)
      expect(validationCart.valid).toBe(false)
      expect(validationCheckout.valid).toBe(false)
    })
  ...
  describe('🧹 Token Cleanup Tests', () => {
    it('should cleanup expired tokens', async () => {
      ...
      // Manually expire the token by setting expiration more than 7 days ago
      // (cleanup only removes tokens expired more than 7 days ago)
    ...
    it('should revoke token', async () => {
      ...
      expect(new Date(revokedToken!.expiresAt).getTime()).toBeLessThan(new Date().getTime())
